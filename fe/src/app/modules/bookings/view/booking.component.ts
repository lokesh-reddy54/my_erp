...
export class BookingViewComponent implements OnInit {
  ...
  employees = [];
  hasUnverifiedEmployees: any = false;
  loadEmployees() {
    console.log("loadEmployees :: ");
    this.service
      .getEmployees(this.booking.clientId)
      .pipe(take(1))
      .subscribe((res) => {
        this.employees = res["data"];

        var unverified = _.filter(this.employees, {
          hasAccess: 1,
          verified: null,
        });
        if (unverified.length) {
          this.hasUnverifiedEmployees = true;
        }
        this.loading = false;
      });
  }
  ...
  cancelResourceBooking(booking) {
    ...
    this.service.saveResourceBooking(data).subscribe((res) => {
      console.log("ResourceBookingsList :: saveResourceBooking : res : ", res);
      if (res["data"]) {
        this.dialogs.success("Resource Booking is cancelled successfully .!");
        this.loadResourceBookings();
        this.getBookingCreditHistory(this.id);
        this.loadInvoices();
      } else {
    ...
    this.service.saveResourceBooking(data).subscribe((res) => {
      ...
      if (res["data"]) {
        this.dialogs.success(
          "Resource Booking is marked as Paid successfully .!"
        );
        this.loadResourceBookings();
      } else {
        this.dialogs.error(res["error"], "Error");
  ...
  sendEmployeeVerification() {
    ...
    this.loading = true;
    this.service.sendEmployeeVerification(data).subscribe((res) => {
      console.log(
        "ResourceBookingsList :: sendEmployeeVerification : res : ",
        res
      );
      if (res["data"]) {
        this.dialogs.success(
          "Verification Request sent to " +
            res["data"] +
  ...
  cancelBooking() {
    ...
    this.dialogs
      .confirm(
        "Are you sure to Cancel this booking for '" +
          this.booking.client.company +
          "' ?"
      )
      .then((event) => {
        if (event.value) {
  ...
  requestExit() {
    ...
    .subscribe(
      (res) => {
          ...
          if (res["data"]) {
            this.dialogs.success(
              "Booking Exit Request is submitted successfully "
            );
            this.loading = false;
  ...
  cancelExitRequest() {
    this.loading = true;
    this.service
      .cancelExitRequest(this.id)
      .pipe(take(1))
      .subscribe(
        (res) => {
          if (res["data"]) {
            this.dialogs.success(
              "Booking Exit request is cancelled successfully "
            );
            this.loading = false;
            this.getBooking();
          } else if (res["error"]) {
  ...
  showExtendContract: any = false;
  extendContract() {
    this.loading = true;
    var data = this.extendContractForm.value;
    data.bookingId = this.id;
    data.exitDate = Utils.ngbDateToMoment(data.exitDate).format("YYYY-MM-DD");
    this.service
      .extendedContract(data)
      .pipe(take(1))
      .subscribe(
        (res) => {
          if (res["data"]) {
            this.dialogs.success("Booking Contract is extended successfully ");
            this.loading = false;
            this.showExtendContract = false;
            this.getBooking();
          } else if (res["error"]) {
            this.dialogs.error(res["error"]);
          }
  ...
  calculateFreeCredits() {
    var freeCreditAmount =
      this.contract.rent * (this.contract.paxPercent / 100);
    var freeCredits = freeCreditAmount / this.contract.paxPrice;
    if (this.contract.paxPercent) {
      this.contract.freeCredits = Math.round(freeCredits);
    }
  }
  ...
  editContract(contract?) {
    ...
    this.service
      .getBuildingMeetingRoomPaxPrice(this.booking.officeId)
      .subscribe((res) => {
        if (res["data"]) {
          this.contract.paxPrice = res["data"];
        }
      });
    var self = this;
  ...
  confirmContract() {
    ...
    this.dialogs
      .confirm(
        "Are you sure to Confirm Contract for this Booking .. ?",
        "Confirm Contract"
      )
      .then(function (event) {
        if (event.value) {
          self.contract.confirmed = true;
  ...
  saveAdditionalInvoice(item?) {
    ...
    this.service
      .saveAdditionalInvoice(additionalInvoice)
      .pipe(take(1))
      .subscribe(
        (res) => {
          self.loading = false;
          if (res["data"]) {
            ...
            if (this.additionalInvoice) {
              additionalInvoice.invoiceService =
                this.additionalInvoice.invoiceService;
            }
            if (!additionalInvoice.id) {
              self.dialogs.success(
                "Contract Additional Invoice is added successfully "
              );
              this.contract.additionalInvoices.push(additionalInvoice);
            } else {
              self.dialogs.success(
                "Contract Additional Invoice is updated successfully "
              );
              this.contract.additionalInvoices = _.reject(
                this.contract.additionalInvoices,
                { id: additionalInvoice.id }
              );
    ...
    this.dialogs
      .confirm(
        "Are you sure to Confirm " +
          contract.kind +
          " Contract for this Booking .. ?",
        "Cancel " + contract.kind + " Contract"
      )
      .then(function (event) {
        if (event.value) {
          self.loading = true;
          self.service
            ...
            .subscribe(
              (res) => {
                if (res["data"]) {
                  self.dialogs.success(
                    contract.kind + " Contract is cancelled successfully "
                  );
                  self.loading = false;
                  self.getBooking();
                } else if (res["error"]) {
  ...
  openAdditionalInvoices(contract) {
    ...
    this.openedModal = this.dialogs.modal(this.additionalInvoicesModal, {
      size: "lg",
    });
    var self = this;
    this.accountsService
  ...
  raiseInvoices() {
    this.loading = true;
    this.service
      .raiseInvoices({ bookingId: this.booking.id })
      .pipe(take(1))
      .subscribe(
        (res) => {
          if (res["data"] != null) {
  ...
  raiseNextMonthInvoice() {
    this.loading = true;
    var self = this;
    this.service
      .raiseNextMonthInvoice({ bookingId: this.booking.id })
      .pipe(take(1))
      .subscribe(
        (res) => {
          self.loading = false;
          ...
          if (res["data"] != null) {
            if (res["data"].id) {
              this.dialogs.success("Next Month Invoice Generated successfully ");
              this.loadInvoices();
            } 
            else if (res["data"]) {
              this.dialogs.info(res["data"]);
            }
            else {
              this.dialogs.info("No invoices are pending to raise. ");
            }
  ...
  updateBookingsLedger() {
    this.loading = true;
    this.service
      .updateBookingsLedger({ bookingIds: [parseInt(this.id)] })
      .pipe(take(1))
      .subscribe(
        (res) => {
          this.getBooking();
  ...
  newResourceBooking() {
    ...
    var dialog = this.dialogs.newResourceBooking(
      this.id,
      this.booking.officeId,
      this.booking.locationId,
      this.booking.client
    );
    dialog.result.then(function (data) {
      if (data && data.id) {
        self.dialogs.success(
          "New resource booking is successful .. !",
          data.name
        );
  ...
  openCreditsHistory() {
    ...
    var self = this;
    this.openedModal = this.dialogs.modal(this.creditsHistoryModal, {
      size: "xlg",
    });
    this.openedModal.result.then(function () {}).catch(function (e) {});
  }
  ...
  exitCalculations: any = {
    earlyExitCharge: { charge: 0 },
    noticePeriodPenality: { charge: 0 },
  };
  calculateExitCharges() {
    ...
    this.exitCalculations.noticePeriodPenality.requestedBeforeDays =
      requestedBeforeDays;
    this.exitCalculations.noticePeriodPenality.stayDuration = stayDuration;
    this.exitCalculations.noticePeriodPenality.noticePeriod =
      this.booking.contract.noticePeriod * 30 + " days";
    if (this.booking.contract.noticePeriod * 30 > requestedBeforeDays) {
      this.exitCalculations.noticePeriodPenality.statement =
        "Notice Period penality will apply as exit is requested before agreed NoticePeriod of " +
        this.booking.contract.noticePeriod * 30 +
        " days as per contract.";
      this.exitCalculations.noticePeriodPenality.period =
        this.booking.contract.noticePeriod * 30 + " days";
      var charge: any = 0;
      if (this.booking.contract.noticePeriodViolationType == "Fixed") {
        this.exitCalculations.noticePeriodPenality.penality =
          "Fixed amount of Rs. " + this.booking.contract.noticePeriodViolation;
        charge = this.booking.contract.noticePeriodViolation;
      } else {
        this.exitCalculations.noticePeriodPenality.penality =
          this.booking.contract.noticePeriodViolation +
          " month(s) rent on prorata basis";
        charge =
          ((this.booking.contract.noticePeriodViolation *
            this.booking.contract.rent) /
            (this.booking.contract.noticePeriod * 30)) *
          requestedBeforeDays;
      }
      ...
      this.exitCalculations.noticePeriodPenality.statement =
        "Notice Period penality wont apply as exit is requested after agreed NoticePeriod of " +
        this.booking.contract.noticePeriod * 30 +
        " days as per contract.";
    }
    this.exitCalculations.earlyExitCharge.lockIn =
      this.booking.contract.lockIn * 30 + " days";
    if (this.booking.contract.lockIn * 30 > stayDuration) {
      this.exitCalculations.earlyExitCharge.statement =
        "Early Exit Charge will apply as exit is requested before agreed LockIn period of " +
        this.booking.contract.lockIn * 30 +
        " days as per contract.";
      this.exitCalculations.earlyExitCharge.period =
        this.booking.contract.lockIn * 30 + " days";
      var charge: any = 0;
      if (this.booking.contract.lockInPenaltyType == "Fixed") {
        this.exitCalculations.earlyExitCharge.penality =
          "Fixed amount of Rs. " + this.booking.contract.lockInPenalty;
        charge = this.booking.contract.lockInPenalty;
      } else {
        this.exitCalculations.earlyExitCharge.penality =
          this.booking.contract.lockInPenalty +
          " month(s) rent on prorata basis";
        charge =
          ((this.booking.contract.lockInPenalty * this.booking.contract.rent) /
            (this.booking.contract.lockIn * 30)) *
          (this.booking.contract.lockIn * 30 - stayDuration);
      }
      this.exitCalculations.earlyExitCharge.charge = charge;
    } else {
      this.exitCalculations.earlyExitCharge.statement =
        "Early Exit charge wont apply as exit is requested after LockIn period of " +
        this.booking.contract.lockIn * 30 +
        " days as per contract.";
    }
    this.booking.exitRequest.monthlyInvoices = this.invoiceAmount;
    this.booking.exitRequest.totalPaid = this.booking.paid;
    this.booking.exitRequest.earlyExitCharge = parseInt(
      this.exitCalculations.earlyExitCharge.charge || 0
    );
    this.booking.exitRequest.noticePeriodPenalty = parseInt(
      this.exitCalculations.noticePeriodPenality.charge || 0
    );

    var assetDamages = _.sumBy(this.booking.exitRequest.acrs, "charge") || 0;
    this.booking.exitRequest.assetDamages = assetDamages * 1.18;

    var otherDeductions =
      _.sumBy(this.booking.exitRequest.deductions, "charge") || 0;
    this.booking.exitRequest.otherDeductions = otherDeductions * 1.18;

    var tdsAmount = _.sumBy(
      _.filter(this.invoices, function (i) {
        return i.isCancelled == 0;
      }),
      "tds"
    );
    var tdsPaidAmount = _.sumBy(
      _.filter(this.invoices, function (i) {
        return i.isCancelled == 0;
      }),
      "tdsPaid"
  ...
  calculateExpectedAmount() {
    var amount =
      this.booking.exitRequest.monthlyInvoices +
      this.booking.exitRequest.earlyExitCharge +
      this.booking.exitRequest.noticePeriodPenalty +
      this.booking.exitRequest.assetDamages +
      this.booking.exitRequest.otherDeductions +
      this.booking.exitRequest.tdsLiability +
      this.booking.exitRequest.deregistrationLiability +
      this.booking.exitRequest.tdsPenalty;
  ...
  approveFinalStatement() {
    var self = this;
    this.dialogs
      .confirm("Are you sure to request for Final Statement Approval . ?")
      .then(function (event) {
        if (event.value) {
          self.loading = true;
          self.service
            .approveFinalStatement(self.booking.exitRequest)
            .pipe(take(1))
            .subscribe((res) => {
              if (res["data"]) {
                self.dialogs.success(
                  "Final Statement is approved successfully .. !"
                );
  ...
  acceptFinalStatement() {
    ...
    this.dialogs
      .confirm(
        "Are you sure to force accept Final Statement on client behalf . ?"
      )
      .then(function (event) {
        if (event.value) {
          self.loading = true;
          ...
          self.service
            .acceptFinalStatement(data)
            .pipe(take(1))
            .subscribe((res) => {
              if (res["data"]) {
                self.dialogs.success(
                  "Final Statement is force accepted successfully .. !"
                );
                self.booking.exitRequest = res["data"];
                self.getBooking();
              } else {
  ...
  deleteAcr(acr) {
    ...
    this.service
      ...
      .pipe(take(1))
      .subscribe((res) => {
        this.booking.exitRequest.acrs = _.without(
          this.booking.exitRequest.acrs,
          acr
        );
        this.savingAcr = false;
        this.calculateExitCharges();
  ...
  deleteDeduction(deduction) {
    ...
    this.service
      ...
      .pipe(take(1))
      .subscribe((res) => {
        this.booking.exitRequest.deductions = _.without(
          this.booking.exitRequest.deductions,
          deduction
        );
        this.savingDeduction = false;
        this.calculateExitCharges();
  ...
  openContraction() {
    ...
    this.openedModal = this.dialogs.modal(this.contractionModal, {
      size: "xlg",
    });
    this.openedModal.result
      .then(function () {
  ...
  checkForValidContraction() {
    ...
    if (
      Utils.ngbDateToMoment(this.contractionDate).isAfter(
        this.minContractionDate
      )
    ) {
  ...
  openContracsHistory() {
    this.service
      .getContracts(this.booking.id)
      .pipe(take(1))
      .subscribe((res) => {
        this.contractsHistory = res["data"];
      });
    this.openedModal = this.dialogs.modal(this.contractsHistoryModal, {
      size: "lg",
    });
    this.openedModal.result.then(function () {}).catch(function (e) {});
  }
  ...
  searchOffices() {
    ...
    // console.log("this.selectedDeskType.type", this.selectedDeskType)
    if(this.selectedDeskType != undefined){
  ...
  selectHotDesks(office, cabin) {
    if (this.contract.type == "HotDesk") {
      ...
      this.selectedOffices = [
        { officeId: office.id, office: office.name, cabins: [_cabin] },
      ];
      console.log(
        "selectHotDesks ::: selectedOffices :: ",
        this.selectedOffices
      );
    }
  }
  updateBookingPrice() {
    console.log("updateBookingPrice:: updateBookingPrice:: ")
    var self = this;
    ...
    _.each(this.selectedOffices, function (o) {
      _.each(o.cabins, function (c) {
        ...
        if (c.price && c.price > 0) {
          self.totalBookingPrice =
            self.totalBookingPrice + c.price * c.desksCount;
          if (c.cabins) {
            self.desks = [];
        ...
        if (self.expansionType && self.expansionType == "Expansion") {
          self.contract.rent =
            self.booking.contract.rent + self.totalBookingPrice;
    ...
    if (self.contract.term == "ShortTerm") {
      console.log("ShortTerm price updating.....!")
      var rent = 0;
      var noOfDays =
        moment(self.booking.endDate).diff(
          moment(self.booking.startDate),
          "days"
        ) + 1;
      var rentPerDay =
        self.totalBookingPrice / moment(self.booking.startDate).daysInMonth();
  ...
  confirmExpansion() {
    ...
    var data = {
      ...
      expansionDate: Utils.ngbDateToMoment(this.expansionDate)
        .add(8, "hours")
        .toDate(),
      desks: this.desks,
  ...
  confirmRelocation() {
    ...
    var data = {
      bookingId: this.booking.id,
      relocationDate: Utils.ngbDateToMoment(this.expansionDate)
        .add(8, "hours")
        .toDate(),
      desks: this.desks,
      selectedCabins: this.selectedCabins ,
      contract: this.contract,
      deskTypes: [this.selectedDeskType.type],
      currentDesks: currentDesks,
    };
    console.log("BookingComponent ::: confirmRelocation :: data : ", data);
    this.loading = true;
    this.service
      .bookingRelocation(data)
      .pipe(take(1))
      .subscribe((res) => {
        this.loading = false;
        if (res["data"]) {
          this.dialogs.success(
            "Booking Relocation has done successfully .. !!"
          );
          this.getBooking();
          this.openedModal.close(res["data"]);
        } else {
  ...
  confirmContraction() {
    ...
    var data = {
      bookingId: this.booking.id,
      contractionDate: Utils.ngbDateToMoment(this.contractionDate)
        .add(8, "hours")
        .toDate(),
      desks: desks,
      contract: contract,
    ...
    this.loading = true;
    this.service
      .bookingContraction(data)
      .pipe(take(1))
      .subscribe((res) => {
        this.loading = false;
        if (res["data"]) {
          this.dialogs.success(
            "Booking Contraction has done successfully .. !!"
          );
          this.getBooking();
          this.openedModal.close(res["data"]);
        } else {
  ...
  vacateOffice() {
    ...
    var data = {
      bookingId: this.booking.id,
      contractId: this.booking.contract.id
    };
    console.log("Vacate office :: contract ::", data)
    var self = this;
    this.dialogs
      .confirm(
        "This isn't termination or a request to leave; it's simply vacating the office. For termination, please refrain from using this procedure... Are you sure you want to just vacate the office..? "
      )
      .then((result) => {
        if (result.value) {
          self.loading = true;
          self.service
            ...
            .subscribe(
              (res) => {
                if (res["data"]) {
                  self.dialogs.success("Vacation of the office is successfull ");
                  self.loading = false;
                  // self.loadInvoices();
                } else if (res["error"]) {
                  self.dialogs.error(res["error"]);
  ...
  companyRegistrationUploadResponse: any = {
    status: "",
    message: "",
    filePath: "",
  };
        ...
        if (res.file) {
          ...
          this.service
            .saveClient({
              ...
              companyRegistrationId: res.id,
            })
            .pipe(take(1))
            .subscribe((res) =>
              this.dialogs.success(
                "Company Registration Certificate uploaded successfully..!!"
              )
            );
        }
      },
  ...
  gstRegistrationUploadResponse: any = {
    status: "",
    message: "",
    filePath: "",
  };
      ...
      (res) => {
        ...
        if (res.file) {
          ...
          this.service
            .saveClient({
              id: this.booking.client.id,
              gstRegistrationId: res.id,
            })
            .pipe(take(1))
            .subscribe((res) =>
              this.dialogs.success(
                "GST Registration Certificate uploaded successfully..!!"
              )
            );
        }
      },
    ...
    this.uploadService.upload(formData).subscribe(
      (res) => {
        ...
        if (res.file) {
          ...
          this.service
            .saveClient({ id: this.booking.client.id, panCardId: res.id })
            .pipe(take(1))
            .subscribe((res) =>
              this.dialogs.success("PAN Card uploaded successfully..!!")
            );
        }
      },
  ...
  exitFormFile: any;
  exitFormFileChange(event) {
    this.exitFormFile = event.target.files[0];
  }
  exitFormUploadResponse: any = { status: "", message: "", filePath: "" };
  exitFormFileError: any;
  uploadExitFormFile() {
    const formData = new FormData();
    formData.append("file", this.exitFormFile);
    this.loading = true;
    this.uploadService.upload(formData).subscribe(
      (res) => {
        this.exitFormUploadResponse = res;
        if (res.file) {
          this.loading = false;
          this.booking.exitRequest.exitForm = res;
          this.exitFormFile = null;
          this.service
            .saveExitRequest({
              id: this.booking.exitRequest.id,
              exitFormId: res.id,
            })
            .pipe(take(1))
            .subscribe((res) =>
              this.dialogs.success("Exit Form uploaded successfully..!!")
            );
        }
      },
      (err) => (this.panCardFileError = err)
  ...
  saveContractTerm(item?) {
    ...
    this.service.saveContractTerm(data).subscribe((res) => {
      if (data.status == "Archived") {
        this.contract.terms = _.reject(this.contract.terms, { id: data.id });
      } else {
        if (!data.id) {
          this.contract.terms.push(res["data"]);
  ...
  max(coordType: "lat" | "lng"): number {
    return Math.max(...this.markers.map((marker) => marker[coordType])) + 0.001;
  }
  min(coordType: "lat" | "lng"): number {
    return Math.min(...this.markers.map((marker) => marker[coordType])) - 0.001;
  }
  markerOut(marker) {
  ...
  signedAgreementUploadResponse: any = {
    status: "",
    message: "",
    filePath: "",
  };
    ...
    this.uploadService.upload(formData).subscribe(
      (res) => {
        ...
        if (res.file) {
          ...
          this.service
            .saveContract({ id: contract.id, signedAgreementId: res.id })
            .pipe(take(1))
            .subscribe((res) =>
              this.dialogs.success(
                "Signed Agreement is uploaded successfully..!!"
              )
            );
        }
      },
  ...
  updateRentFreePeriod() {
    var data = {
      bookingId: this.booking.id,
      contractId: this.booking.contract.id,
      rentFreePeriod: this.booking.contract.rentFreePeriod,
    };
    this.loading = true;
    this.service.updateRentFreePeriod(data).subscribe((res) => {
      this.loading = false;
      if (res["data"]) {
        this.dialogs.success(
          "Invoices are revised with Rent Free Period",
          "RentFree Period Updated"
        );
      } else {
        this.dialogs.error(res["error"], "Error");
      }
