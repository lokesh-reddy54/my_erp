...
@Component({
  selector: 'selfcare-acceptfinalstatement',
  templateUrl: './view.component.html'
})
export class AcceptFinalStatementComponent implements OnInit {
  form: FormGroup;
  ...
  constructor(private route: ActivatedRoute, private dialogs: DialogsService, private service: SelfcareService, private commonService: CommonService) {
  ...
  ngOnInit() {
    ...
    this.service.getInitData(this.id).pipe(take(1)).subscribe(
      res => {
        var data = res['data'];
        this.booking = data;
        document.title = data.company.name + " - " + "Accept Final Statement";
  ...
  acceptFinalStatement(accepted?) {
    ...
    if (accepted) {
        ...
        .then(function(event) {
          if (event.value) {
            ...
            var data = {
              ...
              benificiaryName: self.booking.client.benificiaryName || '',
      ...
      this.dialogs.confirm("Are you sure to Reject this Final Statement .. ?")
        .then(function(event) {
          if (event.value) {
            self.loading = true;
            var data = {
              exitRequestId: self.booking.exitRequest.id,
              fcpStatus: "Rejected",
              rejectedMessage: self.rejectedMessage,
              companyId: self.booking.companyId,
            }
            self.service.acceptFinalStatement(data).pipe(take(1)).subscribe(
              res => {
                if (res['data']) {
                  self.dialogs.success("Final Statement is rejected successfully .. !");
                  self.booking.exitRequest = res['data'];
                } else {
                  self.dialogs.error(res['error']);
                }
