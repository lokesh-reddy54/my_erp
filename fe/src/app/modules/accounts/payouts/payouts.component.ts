...
@Component({
  selector: 'accounts-payouts',
  templateUrl: './payouts.component.html'
})
export class AccountsPayoutComponent implements OnInit {
  ...
  payout: any = {};
  payoutConfig: any = {};
  payoutsFilters: any = { types: [], paymentModes: [], statuses: ["Approved", "PrePaid"] };
  ...
  constructor(private dialogs: DialogsService, private service: AccountsService, private adminService: AdminService, private commonService: CommonService) {
  }
  ...
  ngOnInit() {
    ...
    if (this.commonService.checkAccess("accounts:processPayout")) {
      actions.push({ icon: 'fa fa-dollar', hint: 'Process Payout', code: 'processPayout', style: 'primary', condition: { label: 'status', value: 'Approved' }, also: { label: 'paymentMode', value: 'CashFree' } });
      actions.push({ icon: 'fa fa-dollar', hint: 'Process Payout', code: 'processPayout', style: 'primary', condition: { label: 'status', value: 'FuturePayout' }, also: { label: 'paymentMode', value: 'CashFree' } });
    }
    ...
    if (this.commonService.checkAccess("accounts:submitPayoutDetails")) {
      actions.push({ icon: 'i-Credit-Card', hint: 'Submit Payout Details', code: 'submitPayout', style: 'primary', condition: { label: 'status', value: 'Approved' }, also: { label: 'paymentMode', not: 'CashFree' } });
      actions.push({ icon: 'i-Credit-Card', hint: 'Submit Payout Details', code: 'submitPayout', style: 'primary', condition: { label: 'status', value: 'PrePaid' } });
      actions.push({ icon: 'i-Credit-Card', hint: 'Submit Payout Details', code: 'submitPayout', style: 'primary', condition: { label: 'status', value: 'FuturePayout' } });
      actions.push({ icon: 'fa fa-check', hint: 'Mark as Encashed', code: 'submitPayout', style: 'info', condition: { label: 'status', value: 'Issued' } });
      actions.push({ icon: 'fa fa-refresh', hint: 'Change Payment Method', code: 'submitPayout', style: 'info', condition: { label: 'status', value: 'Approved' }, also: { label: 'paymentMode', value: 'CashFree' } });
      actions.push({ icon: 'fa fa-refresh', hint: 'Reset Error', code: 'resetError', style: 'primary', condition: { label: 'status', value: 'Error' } });
    }
    if (this.commonService.checkAccess("accounts:updatePayoutStatus")) {
      actions.push({ icon: 'fa fa-refresh', hint: 'Update Payout Status', code: 'updatePayoutStatus', style: 'primary', condition: { label: 'paymentStatus', value: 'PENDING' }, also: { label: 'paymentMode', value: 'CashFree' } });
    }
    if (this.commonService.checkAccess("accounts:approvePayout")) {
      actions.push({ icon: 'fa fa-check', hint: 'Approve Payout', code: 'approvePayout', style: 'info', condition: { label: 'status', value: 'Pending' } });
    }
  ...
  totalPayouts: any = 0;
  getPayoutsByStatus() {
    this.service.getPayoutsByStatus({
  ...
  processPayout() {
    ...
    this.service.processPayout(data).pipe(take(1)).subscribe(
      res => {
        if (res['data']) {
  ...
  updatePayoutStatus() {
    ...
    this.service.updatePayoutStatus(data).pipe(take(1)).subscribe(
      res => {
        if (res['data']) {
          self.payoutsList.reset();
        } else {
  ...
  confirmPayout() {
    ...
    this.loading = false;
    this.service.savePayout(data).pipe(take(1)).subscribe(
      res => {
  ...
  rejectPayout() {
    ...
    this.dialogs.confirm("Are you sure to Reject this Payout Item for " + self.payout.benificiary + " ? ")
      .then(function(flag) {
        if (flag) {
          self.service.savePayout(data).pipe(take(1)).subscribe(
            res => {
  ...
  action(event) {
    ...
    if (event.action == 'approvePayout') {
      ...
      //   .then(evt => {
      //     if (evt.value) {
      //       this.service.savePayout({ id: event.item.id, status: 'FuturePayout' }).pipe(take(1))
      //         .subscribe(res => {
      //           if (res['data']) {
