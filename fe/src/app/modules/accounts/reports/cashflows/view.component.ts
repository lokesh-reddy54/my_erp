...
@Component({
  selector: 'accounts-reports-cashflows',
  templateUrl: './view.component.html'
})
export class AccountsCashFlowsReportComponent implements OnInit, AfterViewInit {
  ...
  loadBuildings() {
    this.reportsService.getAR(this.officesFilters).pipe(take(1)).subscribe(
      res => {
        ...
        buildings = _.orderBy(buildings, ['due'], ['desc']);
  ...
  loadOffices(building) {
    ...
    this.reportsService.getAR(this.officesFilters).pipe(take(1)).subscribe(
      res => {
        ...
        offices = _.orderBy(offices, ['due'], ['desc']);
    ...
    if (!this.selectedOffice.bookings) {
      ...
      this.reportsService.getAR(this.officesFilters).pipe(take(1)).subscribe(
        res => {
          ...
          bookings = _.orderBy(bookings, ['due'], ['desc']);
