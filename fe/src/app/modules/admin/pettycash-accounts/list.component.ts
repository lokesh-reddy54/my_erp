...
@Component({
  selector: 'admin-list-pettycash-accounts',
  templateUrl: './list.component.html'
})
export class AdminPettyCashAccountsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.config = {
      columns: [
        { label: 'AccountName', field: 'name', type: 'text', styleClass: 'w-20', sortable: true },
        { label: 'Users', field: 'users', type: 'text', styleClass: 'w-40', sortable: true },
  ...
  userSelected(event) {
    ...
    this.saveAccountUser({ pettyCashAccountId: this.account.id, userId: event.id });
  }
  ...
  save() {
    ...
    this.service.savePettyCashAccount(account).pipe(take(1)).subscribe(
      res => {
        if (res['data']) {
          this.account = res['data'];
          self.dialogs.success("PettyCash Account '" + this.account.name + "' is saved successfully ");
          self.loading = false;
          self.list.reset();
  ...
  saveAccountUser(userAccount) {
    this.service.savePettyCashAccountUser(userAccount).subscribe(res => {
      if (res['data'] && res['data']['active']) {
        this.dialogs.success("User '" + this.selectedUser.name + "' is added to PettyCash Account");
        ...
        this.dialogs.success("User '" + userAccount.user.name + "' is removed from PettyCash Account");
