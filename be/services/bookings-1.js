  ...
  try {
    ...
    booking.futureContract = await Contract.findOne({
      where: {
        ...
        status: { $ne: "Cancelled" },
       // effectiveDate: { $gt: booking.contract.effectiveDate },
      },
      include: [
        {
          as: "additionalInvoices",
...
service.requestExit = async (data, username) => {
  try {
    ...
    booking.set("ended", utils.moment(data.exitDate).format("YYYY-MM-DD"));
...
service.listClients = async (data) => {
  try {
    ...
    var result = await Client.findAll({
      ...
      where: where,
      include: [{ as: "booking", model: Booking, attributes: ["id"] }],
      offset: data.offset || 0,
      limit: data.limit || 10,
    });
    return result;
  ...
  try {
    ...
    if (data.id) {
      if (!data.tdsSubmitted) {
        ...
        data.gst = data.taxableAmount * 0.18;
        ...
        //invoice.tds > 0 && 
        if (data.taxableAmount>19000 && data.invoiceServiceId !=6) {
      ...
      if (invoice.isLiability) {
        refNo = await systemUtils.getRefNo(
          "Liability",
          booking.id,
          invoice.date,
          booking.company
...
service.generateInvoicePdf = async (invoice) => {
  try {
    ...
    var items = [];
    _.each(invoice.items, function (i) {
      var description = invoice.invoiceService
        ? invoice.invoiceService.name
        : "";
      if (
      ...
      items.push({
        ...
        igst: i.igst.toLocaleString(),
        cgst: i.cgst.toLocaleString(),
        sgst: i.sgst.toLocaleString(),
        tds: i.tds.toLocaleString(),
    ...
    var data = {
      ...
      dueDate: utils.moment(invoice.dueDate).format("MMM DD, YYYY"),
      place:
        invoice.booking.company.city + ", " + invoice.booking.company.state,
    ...
    var tmpFile = await services.parseContent("pdfs/invoice.html", data);
    var doc = await services.createDoc(
...
service.raiseNextMonthInvoice = async (data) => {
  try{
    ...
    var invoice = await Invoice.findOne({ where: { bookingId: data.bookingId, type: "OfficeRent" },
    ...
    return `Sept Month Invoice is Already Generated`;
    else
      ...
      if (data.invoiceId) {
        ...
        if (futureContract) {
          ...
          if (booking.contract.isSqftRent) {
            ...
            itemDescription =
              "Rent2 for " +
              booking.contract.sqFt +
              " SqFt at price of Rs." +
              booking.contract.sqFtPrice +
              " per SqFt in " +
              booking.offices +
              " <br> for period of " +
              utils
        ...
        console.log("Futurecontract after iteeeem deksts issue treacker:::",item );
          ...
          itemDescription =
            ...
            " - " +
            utils.moment(invoice.endDate).format("MMM DD, YYYY");
            itemDescription = "Advance Business Services for the Month of " +
  ...
  try {
    ...
    var sql =
      `SELECT b.id,i.id invoiceId,i.status,i.refNo, d.date_field invoiceDate, b.started, b.ended, b.status  
              ...
              where 1=1 and b.status in ("Active","Exiting") and (b.ended is null or b.ended> ` +
      dateSql +
      `) 
              and (i.id is null or i.refNo is null ` +
      ignoreCancelled +
      `) ` +
      bookingIdSql +
      `
   ...
   log.write("BookingService ::: raiseInvoices :: bookings : ", bookings);
    ...
    for (var i = 0; i < bookings.length; i++) {
      ...
      for (var j = 0; j < booking.invoices.length; j++) {
        ...
        if (invoice.status == null || invoice.status == "Cancelled") {
    ...
    BookedDesk.update(
      {
        //status: "Releasing",
        status: "Released",
        ended: utils.moment(contract.effectiveDate).add(-1, "days").toDate(),
      },
      { where: { bookingId: booking.id } }
    );
...
service.getContractRenewalBookings = async (data) => {
  try {
    var sql = `select b.id, ct.id contractId, ifnull(c.company,c.name) company,b.id bookingid, b.offices, b.started, b.contractId, ct.effectiveDate, ct.rent, (ct.rent*1.1) newRent, DATE_ADD(ct.effectiveDate,INTERVAL 11 MONTH) newEffectiveDate
    from bookings b, clients c, contracts ct where b.clientId=c.id and b.contractId=ct.id and b.status in ('Active')  and ct.status!='Cancelled'  and
